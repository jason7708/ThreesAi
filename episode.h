...
public:
	void train_update(weight_agent& weight_table, learning_agent& _alpha){
		for(size_t i = ep_moves.size()-2; i>=9; i-=2){
			float reward_st_1 = 0, value_st_1 = 0, value_st = 0;

			if(i != ep_moves.size() - 2) {
				reward_st_1 = ep_moves[i+2].reward;
				board &x = ep_moves[i+2].board_now;
				value_st_1 = weight_table.predictValue(x);
			}	

			board &y = ep_moves[i].board_now;
			value_st = weight_table.predictValue(y);

			float error = (value_st_1 - value_st + reward_st_1) * _alpha.get_alpha() / 192;
			weight_table.updateWeight(y, error);
		}
	}
public:
